...
public class UsersService {
	...
	public Users editUser(Users users, Long userId) {
		...
		// userRepository.save(users);
		userRepository.updateUserById(userId, users.getZipcode(), users.getEmail(), users.getName(), users.getPassword());
		// userRepository.updateUserById(userId, zipcode, useremailId, name, password)
	...
	public void deleteRole(Long userid) {
		userRoleRepository.findById(userid).orElseThrow(() -> new ResponseStatusException(
		HttpStatus.NOT_FOUND, "User not found!"));
